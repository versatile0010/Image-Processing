...
void calc_lbp(Mat src, int sx, int sy, int face_height, int face_width, float* lbp_histo) {
	...
	float* histo = (float*)calloc(256 * (WIN / (BLK / 2) - 1) * (WIN / (BLK / 2) - 1), sizeof(float));
	Mat img = src.clone();
	...
	for (int y = 0; y <= WIN - BLK; y += BLK / 2) {
		for (int x = 0; x <= WIN - BLK; x += BLK / 2) {
			...
			for (int i = 0; i < 256; i++) {
				if (sum == 0)
					lbp_histo[256 * cnt + i] = 0;
				else
					lbp_histo[256 * cnt + i] = histo[i] / sum;
			}
			cnt++;
		}
	}
...
void main(int ac, char** av) {
	...
	float* target_histo = (float*)calloc(256 * (WIN / (BLK / 2) - 1) * (WIN / (BLK / 2) - 1), sizeof(float));
	float* ref_histo = (float*)calloc(256 * (WIN / (BLK / 2) - 1) * (WIN / (BLK / 2) - 1), sizeof(float));
	...
	while (true) {
		...
		if (faces.size() > 0 && registered == true) {
			for (int i = 0; i < faces.size(); i++) {
				float score = 0.0;
				fill_zeros(target_histo, 256 * (WIN / (BLK / 2) - 1) * (WIN / (BLK / 2) - 1));
				...
				score = get_cosine_sim(target_histo, ref_histo, 256 * (WIN / (BLK / 2) - 1) * (WIN / (BLK / 2) - 1));
